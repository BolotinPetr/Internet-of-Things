...
$("#led-link-auto").on('click', function(e){
    socket.emit('auto', {value: 0, userId: userId});
});
...
socket.on('auto', function(msg) {
    if(msg.value === false) {
        $('#messages').prepend($('<li>AUTO: OFF<span> - '+msg.userId+'</span></li>'));
        $("#led-container").removeClass("on-auto");
        $("#led-container").addClass("off-auto");
        $("#led-container span").text("AUTO OFF");
    }
    else if(msg.value === true) {
        $('#messages').prepend($('<li>AUTO: ON<span> - '+msg.userId+'</span></li>'));
        $("#led-container").removeClass("off-auto");
        $("#led-container").addClass("on-auto");
        $("#led-container span").text("AUTO ON");
    }
});
...
socket.on('connected users', function(msg) {
    ...
    for(var i = 0; i < msg.length; i++) {
        ...
        if(msg[i] == userId)
            $('#user-container').append($("<div id='" + msg[i] + "' class='my-circle'><span>"+msg[i]+"</span></div><br>"));
        else
            $('#user-container').append($("<div id='" + msg[i] + "' class='user-circle'><span>"+msg[i]+"</span></div><br>"));
    }
});
